...
- Each speech's checked-in transcript has non-overlapping timings (Stanford: 144 lines, JFK: 55 lines, Eisenhower: 78 lines). The page highlights and scrolls to the active sentence; clicking a line seeks to it.
...
- **Next** can advance and persist position without a recording; full completion and MP3 export still require a take for every line of the active speech.
- Pushes to `main` deploy a `/shadowing/` production build through GitHub Actions to `https://signofthefour.github.io/shadowing/`.

## Latest verification

- `npm test`: 35 tests passed (12 files).
...
1. Run `npm run dev` and practice the first few sentences of each of the three speeches with a real microphone. Check that playback stops at a natural sentence boundary and that Listen/Redo/Next work.
2. Manually check timing at the three story transitions and the final sentence of Stanford's speech. Correct any inaccurate timestamps in `src/data/speeches/stanfordSpeech.generated.json`.
3. JFK's and Eisenhower's transcripts have not had this manual pass yet. In particular, JFK's opening lines (`line-001`/`line-002` in `src/data/speeches/jfkInaugural.generated.json`) are known to be split mid-clause on the abbreviation "Mr." (a limitation of the shared sentence-splitter, not specific to this speech) — spot-check both new speeches end-to-end and correct their source text/timings the same way as Stanford's.
4. Complete a real-microphone session far enough to verify restored audio and audible MP3 playback/export; automated browser tests use deterministic media fakes.
...
- `src/app/App.tsx` — Library/Practice shell: shows the speech library or the active practice flow, remembers the last-opened speech in localStorage.
